...
std::atomic<bool> mouseHookEnabled(true);

PROCESS_INFORMATION pi = {0};

struct Settings {
    bool enableUntoggleButton = true;
    bool onlyLeftClick = true;
    ...
    std::vector<std::wstring> toggleButtonKeys = {L"XMOUSE2"};
...
std::map<std::wstring, int> keyMap = {
    ...
    {L"XMOUSE1", VK_XBUTTON1}, {L"XBUTTON1", VK_XBUTTON1}, {L"BACK", VK_XBUTTON1},
    {L"XMOUSE2", VK_XBUTTON2}, {L"XBUTTON2", VK_XBUTTON2}, {L"FORWARD", VK_XBUTTON2},
    {L"MOUSE4", VK_XBUTTON1}, {L"MOUSE5", VK_XBUTTON2}
};
...
Settings LoadSettings() {
    ...
    settings.enableUntoggleButton = ExtractJsonBool(cleanJson, L"enableUntoggleButton");
    settings.onlyLeftClick = ExtractJsonBool(cleanJson, L"onlyLeftClick");
    ...
    settings.toggleButtonKeys = ExtractJsonStringArray(cleanJson, L"toggleButtonKeys");
    ...
    settings.mouseCheckInterval = ExtractJsonInt(cleanJson, L"mouseCheckInterval");
...
void StopAppByName() {
    ...
    if (Process32FirstW(hSnap, &pe)) {
        do {
            if (std::wstring(pe.szExeFile) == L"app.exe") {
...
class LagFreeMouseHandler {
private:
    std::atomic<bool> toggleButtonHeld{false};
    std::atomic<bool> enabled{true};
    std::atomic<bool> buttonReleased{false};
    std::thread monitorThread;
    int toggleButtonVK = VK_XBUTTON2;
    
public:
    static LagFreeMouseHandler& GetInstance() {
        static LagFreeMouseHandler instance;
        return instance;
    }
    ...
    void UpdateToggleButton() {
        toggleButtonVK = currentSettings.toggleButtonKeys.empty() ? VK_XBUTTON2 : StringToVK(currentSettings.toggleButtonKeys[0]);
        if (toggleButtonVK == 0) {
            toggleButtonVK = VK_XBUTTON2;
        }
    }
    
    void ReleaseToggleButton() {
        INPUT input[2] = {0};
        int inputCount = 0;
        
        if (toggleButtonVK == VK_XBUTTON1 || toggleButtonVK == VK_XBUTTON2) {
            input[0].type = INPUT_MOUSE;
            input[0].mi.dwFlags = (toggleButtonVK == VK_XBUTTON1) ? MOUSEEVENTF_XDOWN : MOUSEEVENTF_XDOWN;
            input[0].mi.mouseData = (toggleButtonVK == VK_XBUTTON1) ? XBUTTON1 : XBUTTON2;
            
            input[1].type = INPUT_MOUSE;
            input[1].mi.dwFlags = (toggleButtonVK == VK_XBUTTON1) ? MOUSEEVENTF_XUP : MOUSEEVENTF_XUP;
            input[1].mi.mouseData = (toggleButtonVK == VK_XBUTTON1) ? XBUTTON1 : XBUTTON2;
            
            inputCount = 2;
        } else {
            input[0].type = INPUT_KEYBOARD;
            input[0].ki.wVk = toggleButtonVK;
            input[0].ki.dwFlags = 0;
            
            input[1].type = INPUT_KEYBOARD;
            input[1].ki.wVk = toggleButtonVK;
    ...
    void StartMonitoring() {
        ...
        monitorThread = std::thread([this]() {
            ...
            while (!shouldExit && enabled) {
                UpdateToggleButton();
                
                bool currentToggleButtonState = (GetAsyncKeyState(toggleButtonVK) & 0x8000) != 0;
                bool currentLeftButtonState = (GetAsyncKeyState(VK_LBUTTON) & 0x8000) != 0;
                bool currentRightButtonState = (GetAsyncKeyState(VK_RBUTTON) & 0x8000) != 0;
                
                if (currentSettings.enableUntoggleButton) {
                    if (currentToggleButtonState && !lastToggleButtonState) {
                    ...
                    if (toggleButtonHeld && !buttonReleased) {
                        if (currentSettings.onlyLeftClick) {
                            if (currentLeftButtonState && !lastLeftButtonState) {
                                ReleaseToggleButton();
                            }
                        } else {
                            if ((currentLeftButtonState && !lastLeftButtonState) || 
                                (currentRightButtonState && !lastRightButtonState)) {
                                ReleaseToggleButton();
                            }
                        }
                    }
                    
                    if (buttonReleased && !currentToggleButtonState && !wasTogglePressed) {
                        buttonReleased = false;
                    }
                }
                ...
                Sleep(currentSettings.mouseCheckInterval);
            }
        });
    }
    ...
    while (!shouldExit) {
        ...
        if (GetFileAttributesExW(settingsPath.c_str(), GetFileExInfoStandard, &fileData)) {
            if (CompareFileTime(&fileData.ftLastWriteTime, &lastWriteTime) != 0) {
                ...
                UpdateSettings();
                LagFreeMouseHandler::GetInstance().SetEnabled(currentSettings.enableUntoggleButton);
            }
        }
...
int WINAPI wWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, PWSTR pCmdLine, int nCmdShow) {
    ...
    LagFreeMouseHandler::GetInstance().SetEnabled(currentSettings.enableUntoggleButton);
    LagFreeMouseHandler::GetInstance().StartMonitoring();
    ...
    shouldExit = true;
    StopApp();
    LagFreeMouseHandler::GetInstance().StopMonitoring();
